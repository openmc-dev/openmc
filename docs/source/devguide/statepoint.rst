...
*do i = 1, n_tallies*
    ...
    **integer(4) tallies(i) % next_filter_bin - 1**

        Actual number of filter bins in the results array during OTF allocation

    *do j = 1, tallies(i) % next_filter_bin - 1*

        **integer(4) tallies(i) % reverse_filter_index_map % get_key(j)**

            Mapping of local results array bin to real filter bin
