...
Git submodules are no longer used for fmt, pugixml, and Catch2. CMake now uses
installed packages when available and downloads pinned sources otherwise. The
``OPENMC_FORCE_VENDORED_LIBS`` CMake option has been replaced by
