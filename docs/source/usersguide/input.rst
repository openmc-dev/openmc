...
out the file and "false" will not.
  ...
  :distribmats:
    Write out binary or HDF5 files containing the compositions of each
...
An enhanced feature has been implemented to allow for various properties of the
material in distributed cells, called distribmats (distributed materials). That
means, if one material is filled to a cell which has multiple instances, each
material instance can have different density or nuclide compositions. One of
two attributes/sub-elements called ``distributed_density`` and ``compositions``
can be used to specify distributed materials.

  :distributed_density:
    An alternative element to ``density`` to specify distributed densities of
    this material. This element has two attributes/sub-elements ``values`` and
    ``units``. The ``values`` element is a list of numeric values of all the
    densities in distributed materials. The number of values in ``values``
    attribute must be one or the same as the number of distributed instances.
  ...
  :compositions:
    An element to specify different compositions in distributed materials. Note
    that the nulcides in distributed materials are the same and the percent of
    these nulcides can be different. This ``compositions`` element has
    attributes/sub-elements called ``units``, ``values`` and ``nuclide`` or
    ``element``. Elements ``nuclide`` and ``element`` are similar with the
    normal material input except there is no ``ao`` or ``wo`` attribute anymore.

    :units:
      Whether atom percent (``ao``) or weight percent(``wo``) being used in
      ``values`` element.

      *Default*: None
      
    :values:
      A list of numeric values of atom or weight percents of all nuclides in
      distributed materials. The number of values is equal to product of the
      number of nuclides and the number of distributed instances. And the
      nuclides percent are first priority in the list.

      *Default*: None

    :nuclide:
      Same as element ``nuclide`` in element ``material`` except there are no
      ``ao`` and ``wo`` attributes.

      *Default*: None
      
    :element:
      Same as element ``element`` in element ``material`` except there is no
      ``ao`` attribute.
    ...
    .. code-block:: xml
        ...
        <material id="1">
          ...
          <compositions>
            <units>ao</units>
            <nuclide name="U-235" xs="71c"/>   
            <nuclide name="U-238" xs="71c"/> 
            <values>
             1.0 5.0
             1.0 1.0
             1.0 1.0
...
.. Note:: Only one of the two elements can be used at a time.

Furthermore, the compositions in distributed materials can be read from an
external HDF5 file and necessary data are to be loaded into memory during
simulation, called OTF(on-the-fly) distributed materials. The HDF5 file must be
named ``materials.h5``. Composition data of one material is stored as a group,
which contains datasets ``comps`` , ``n_instances`` and ``n_nuclides``. Note
that the composition data read from HDF5 file must be atom percent. The group
name is specified in an elemeng called ``otf_file_path`` within element
``compositions``. One example would be as follows:
    ...
    .. code-block:: xml
        ...
        <material id="1">
          ...
          <compositions>
            <otf_file_path>mat1</otf_file_path>
            <nuclide name="U-235" xs="71c"/>   
            <nuclide name="U-238" xs="71c"/> 
          </compositions>
        </material>

.. Note:: A HDF5 file of distributed materials named ``materials-out.h5`` is
          written by default when the distribmats feature is activated. The
          format of this output file is the same as the ``materials.h5``.
