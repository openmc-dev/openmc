...
If no surface IDs are declared, particles crossing any surface of the model
will be banked::
...
a cell and that are either coming from or going to this specific cell::
...
In this example, particles that are crossing any surface delimiting cell 1 will
...
cell 1 or going to cell 1 will be banked.

To account specifically for particles leaving or entering a given cell, ``cellfrom``
and ``cellto`` are also available to respectively account for particles coming
from a cell::
