"""
LCT006-9: Tank-Type Critical Assembly (TCA) 16x16 rods with 2.15 cm pitch; Hc=78.67 cm
...
surf3 = openmc.ZPlane(surface_id=3, z0=78.67)

# DX=DY=16*2.15        = 34.40 (core planar bdy)
# Box (6 planes): xmin=-17.2, xmax=17.2, ymin=-17.2, ymax=17.2, zmin=-30.0, zmax=144.15
surf4_xmin = openmc.XPlane(surface_id=10000, x0=-17.2)
surf4_xmax = openmc.XPlane(surface_id=10001, x0=17.2)
surf4_ymin = openmc.YPlane(surface_id=10002, y0=-17.2)
surf4_ymax = openmc.YPlane(surface_id=10003, y0=17.2)
surf4_zmin = openmc.ZPlane(surface_id=10004, z0=-30.0)
surf4_zmax = openmc.ZPlane(surface_id=10005, z0=144.15)

# DX=DY=16*2.15 + 2*30 = 94.40 (refl planar bdy)
# Box (6 planes): xmin=-47.2, xmax=47.2, ymin=-47.2, ymax=47.2, zmin=-30.0, zmax=144.15
surf5_xmin = openmc.XPlane(surface_id=10006, x0=-47.2)
surf5_xmax = openmc.XPlane(surface_id=10007, x0=47.2)
surf5_ymin = openmc.YPlane(surface_id=10008, y0=-47.2)
surf5_ymax = openmc.YPlane(surface_id=10009, y0=47.2)
...
# Unit 2: Array of 16x16 fuel rods with 2.15 cm pitch
# Lattice for unit 2: 16x16 array
# Pitch: (2.150000, 2.150000) cm
# Lower left: (-17.2, -17.2)

# Lattice filled with universe1
lattice2 = openmc.RectLattice(lattice_id=2)
lattice2.lower_left = [-17.2, -17.2]
lattice2.pitch = [2.150000, 2.150000]
lattice2.universes = [[universe1]*16]*16
...
source.space = openmc.stats.Point((1.1, 1.1, 39.335))
