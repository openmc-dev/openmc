...
lat.lower_left = [-32.13, -32.13]
lat.pitch = [21.42, 21.42]
lat.universes = [[c, c, r], [c, c, r], [r, r, r]]
...
cells["Upper Reflector Lattice"].region = (
    +surfaces["Core x-min"]
    & -surfaces["Core x-max"]
    & +surfaces["Core y-min"]
    & -surfaces["Core y-max"]
    & +surfaces["Core z-max"]
    & -surfaces["Upper Reflector z-max"]
)

cells["Lower Reflector"] = openmc.Cell(name="Lower Reflector")
cells["Lower Reflector"].fill = materials["Moderator"]
cells["Lower Reflector"].region = (
    +surfaces["Core x-min"]
    & -surfaces["Core x-max"]
    & +surfaces["Core y-min"]
    & -surfaces["Core y-max"]
    & -surfaces["Core z-min"]
    & +surfaces["Lower Reflector z-min"]
)

# Create core universes
universes["Core"] = openmc.Universe(universe_id=0, name="Core")
universes["Core"].add_cells(
    [cells["Core Lattice"], cells["Upper Reflector Lattice"], cells["Lower Reflector"]]
)
