...
public:
  ...
  MgxsInterface(const std::string& path_cross_sections,
    ...
    const vector<vector<xsfloat>> xs_temps);
  ...
  vector<vector<xsfloat>> get_mat_kTs();
  ...
  vector<vector<xsfloat>> xs_temps_to_read_;      // temperatures used
  std::string cross_sections_path_; // path to MGXS h5 file
  vector<Mgxs> nuclides_;
  vector<Mgxs> macro_xs_;
  vector<xsfloat> energy_bins_;
  vector<xsfloat> energy_bin_avg_;
  vector<xsfloat> rev_energy_bins_;
  vector<vector<xsfloat>> nuc_temps_; // all available temperatures
};
