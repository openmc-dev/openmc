...
public:
  ...
  // Binary ops: tensor op tensor (same type, with broadcasting support)
  // When shapes match, uses fast element-wise loop. When shapes are
  // broadcast-compatible (dims are equal or one is 1), uses broadcast logic.
  xtensor operator+(const xtensor& o) const
  {
    ...
    return broadcast_op(o, [](T a, T b) { return a + b; });
  }
  xtensor operator-(const xtensor& o) const
  {
    if (shape_ == o.shape_) {
  ...
  // Assignment from broadcast_view (declared here, defined after broadcast_view)
  template<std::size_t BN>
  xtensor& operator=(const broadcast_view<T, BN>& bv);

private:
  // Broadcasting binary op helper for same-type xtensor<T,N> with
  // broadcast-compatible shapes (each dim equal or one of them is 1).
  // Output shape takes the max of each dimension.
  template<typename Op>
  xtensor broadcast_op(const xtensor& o, Op op) const
  {
    shape_type out_shape;
    shape_type strides_a, strides_b;
    // Compute output shape, strides for a and b (0 for broadcast dims)
    for (std::size_t d = 0; d < N; ++d)
      out_shape[d] = std::max(shape_[d], o.shape_[d]);
    // Compute row-major strides for output shape
    strides_a[N - 1] = 1;
    strides_b[N - 1] = 1;
    for (int d = static_cast<int>(N) - 2; d >= 0; --d) {
      strides_a[d] = strides_a[d + 1] * shape_[d + 1];
      strides_b[d] = strides_b[d + 1] * o.shape_[d + 1];
    }
    xtensor r(out_shape);
    std::size_t total = r.size();
    for (std::size_t flat = 0; flat < total; ++flat) {
      // Decompose flat index into multi-dim indices for output
      std::size_t rem = flat;
      std::size_t off_a = 0, off_b = 0;
      for (std::size_t d = 0; d < N; ++d) {
        std::size_t stride_out = 1;
        for (std::size_t d2 = d + 1; d2 < N; ++d2)
          stride_out *= out_shape[d2];
        std::size_t idx = rem / stride_out;
        rem %= stride_out;
        off_a += (shape_[d] == 1 ? 0 : idx) * strides_a[d];
        off_b += (o.shape_[d] == 1 ? 0 : idx) * strides_b[d];
      }
      r.data_[flat] = op(data_[off_a], o.data_[off_b]);
    }
    return r;
  }
...
//==============================================================================
// newaxis sentinel type
//==============================================================================

struct xnewaxis_type {};

inline xnewaxis_type newaxis()
{
  return {};
}

//==============================================================================
// broadcast_view<T, N>: a read-only view with stride-based broadcasting
//==============================================================================
// Dimensions with stride=0 broadcast (repeat) along that axis.
...
public:
  broadcast_view(
    const T* data, std::array<std::size_t, N> shape,
  ...
  const T& operator()(std::size_t i0) const
  {
  ...
  const T& operator()(std::size_t i0, std::size_t i1) const
  {
  ...
  const T& operator()(std::size_t i0, std::size_t i1, std::size_t i2) const
  {
...
// Helper: compute row-major strides for an xtensor shape
template<std::size_t N>
std::array<std::size_t, N> compute_strides(const std::array<std::size_t, N>& shape)
{
  std::array<std::size_t, N> strides;
  strides[N - 1] = 1;
...
//--- view() overloads with newaxis ---

// view(1D, all, newaxis) → broadcast_view<T,2>
// shape [A] → [A, 1], strides [1, 0]
template<typename T>
broadcast_view<T, 2> view(const xtensor<T, 1>& a, xall_type, xnewaxis_type)
{
  return {a.data(), {a.shape(0), std::size_t(1)}, {std::size_t(1), std::size_t(0)}};
}

// view(1D, all, newaxis, newaxis) → broadcast_view<T,3>
// shape [A] → [A, 1, 1], strides [1, 0, 0]
template<typename T>
broadcast_view<T, 3> view(
  const xtensor<T, 1>& a, xall_type, xnewaxis_type, xnewaxis_type)
{
  return {a.data(), {a.shape(0), std::size_t(1), std::size_t(1)},
    {std::size_t(1), std::size_t(0), std::size_t(0)}};
}

// view(2D, all, newaxis, all) → broadcast_view<T,3>
// shape [A, G] → [A, 1, G], strides [G, 0, 1]
template<typename T>
broadcast_view<T, 3> view(
  const xtensor<T, 2>& a, xall_type, xnewaxis_type, xall_type)
{
  auto s = compute_strides<2>(a.shape());
  return {a.data(), {a.shape(0), std::size_t(1), a.shape(1)},
    {s[0], std::size_t(0), s[1]}};
}

// view(2D, all, all, newaxis) → broadcast_view<T,3>
// shape [A, G] → [A, G, 1], strides [G, 1, 0]
template<typename T>
broadcast_view<T, 3> view(
  const xtensor<T, 2>& a, xall_type, xall_type, xnewaxis_type)
{
  auto s = compute_strides<2>(a.shape());
  return {a.data(), {a.shape(0), a.shape(1), std::size_t(1)},
    {s[0], s[1], std::size_t(0)}};
}

// view(2D, all, newaxis, newaxis, all) → broadcast_view<T,4>
// shape [A, G] → [A, 1, 1, G], strides [G, 0, 0, 1]
template<typename T>
broadcast_view<T, 4> view(const xtensor<T, 2>& a, xall_type, xnewaxis_type,
  xnewaxis_type, xall_type)
{
  auto s = compute_strides<2>(a.shape());
  return {a.data(), {a.shape(0), std::size_t(1), std::size_t(1), a.shape(1)},
    {s[0], std::size_t(0), std::size_t(0), s[1]}};
}

// view(2D, all, all, newaxis, newaxis) → broadcast_view<T,4>
// shape [A, D] → [A, D, 1, 1], strides [D, 1, 0, 0]
template<typename T>
broadcast_view<T, 4> view(const xtensor<T, 2>& a, xall_type, xall_type,
  xnewaxis_type, xnewaxis_type)
{
  auto s = compute_strides<2>(a.shape());
  return {a.data(), {a.shape(0), a.shape(1), std::size_t(1), std::size_t(1)},
    {s[0], s[1], std::size_t(0), std::size_t(0)}};
}

// view(3D, all, newaxis, all, all) → broadcast_view<T,4>
// shape [A, G1, G2] → [A, 1, G1, G2], strides [G1*G2, 0, G2, 1]
template<typename T>
broadcast_view<T, 4> view(const xtensor<T, 3>& a, xall_type, xnewaxis_type,
  xall_type, xall_type)
{
  auto s = compute_strides<3>(a.shape());
  return {a.data(),
    {a.shape(0), std::size_t(1), a.shape(1), a.shape(2)},
    {s[0], std::size_t(0), s[1], s[2]}};
}

// view(3D, all, all, newaxis, all) → broadcast_view<T,4>
// shape [A, D, G] → [A, D, 1, G], strides [D*G, G, 0, 1]
template<typename T>
broadcast_view<T, 4> view(const xtensor<T, 3>& a, xall_type, xall_type,
  xnewaxis_type, xall_type)
{
  auto s = compute_strides<3>(a.shape());
  return {a.data(),
    {a.shape(0), a.shape(1), std::size_t(1), a.shape(2)},
    {s[0], s[1], std::size_t(0), s[2]}};
}

// view(3D, all, all, all, newaxis) → broadcast_view<T,4>
// shape [A, D, G] → [A, D, G, 1], strides [D*G, G, 1, 0]
template<typename T>
broadcast_view<T, 4> view(const xtensor<T, 3>& a, xall_type, xall_type,
  xall_type, xnewaxis_type)
{
  auto s = compute_strides<3>(a.shape());
  return {a.data(),
    {a.shape(0), a.shape(1), a.shape(2), std::size_t(1)},
    {s[0], s[1], s[2], std::size_t(0)}};
}

//--- Arithmetic ops for broadcast_view ---

// broadcast_view<T,3> * broadcast_view<T,3> → xtensor<T,3>
template<typename T>
xtensor<T, 3> operator*(
  const broadcast_view<T, 3>& a, const broadcast_view<T, 3>& b)
{
  auto sa = a.shape();
  std::array<std::size_t, 3> out_shape;
  for (int i = 0; i < 3; ++i)
    out_shape[i] = std::max(sa[i], b.shape(i));
  xtensor<T, 3> r(out_shape);
  for (std::size_t i = 0; i < out_shape[0]; ++i)
    for (std::size_t j = 0; j < out_shape[1]; ++j)
      for (std::size_t k = 0; k < out_shape[2]; ++k)
        r(i, j, k) = a(i, sa[1] == 1 ? 0 : j, sa[2] == 1 ? 0 : k) *
                      b(i, b.shape(1) == 1 ? 0 : j, b.shape(2) == 1 ? 0 : k);
  return r;
}

// broadcast_view<T,4> * broadcast_view<T,4> → xtensor<T,4>
template<typename T>
xtensor<T, 4> operator*(
  const broadcast_view<T, 4>& a, const broadcast_view<T, 4>& b)
{
  std::array<std::size_t, 4> out_shape;
  for (int i = 0; i < 4; ++i)
    out_shape[i] = std::max(a.shape(i), b.shape(i));
  xtensor<T, 4> r(out_shape);
  for (std::size_t i = 0; i < out_shape[0]; ++i)
    for (std::size_t j = 0; j < out_shape[1]; ++j)
      for (std::size_t k = 0; k < out_shape[2]; ++k)
        for (std::size_t l = 0; l < out_shape[3]; ++l)
          r(i, j, k, l) =
            a(i, a.shape(1) == 1 ? 0 : j, a.shape(2) == 1 ? 0 : k,
              a.shape(3) == 1 ? 0 : l) *
            b(i, b.shape(1) == 1 ? 0 : j, b.shape(2) == 1 ? 0 : k,
              b.shape(3) == 1 ? 0 : l);
  return r;
...
// broadcast_view<T,3> * xtensor<T,3> → xtensor<T,3>
template<typename T>
xtensor<T, 3> operator*(
  const broadcast_view<T, 3>& bv, const xtensor<T, 3>& t)
{
  auto s = t.shape();
  xtensor<T, 3> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        r(i, j, k) = bv(i, bv.shape(1) == 1 ? 0 : j,
                       bv.shape(2) == 1 ? 0 : k) *
                      t(i, j, k);
  return r;
}

// xtensor<T,3> * broadcast_view<T,3> → xtensor<T,3>
template<typename T>
xtensor<T, 3> operator*(
  const xtensor<T, 3>& t, const broadcast_view<T, 3>& bv)
{
  return bv * t;
}

// broadcast_view<T,4> * xtensor<T,4> → xtensor<T,4>
template<typename T>
xtensor<T, 4> operator*(
  const broadcast_view<T, 4>& bv, const xtensor<T, 4>& t)
{
  auto s = t.shape();
  xtensor<T, 4> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        for (std::size_t l = 0; l < s[3]; ++l)
          r(i, j, k, l) =
            bv(i, bv.shape(1) == 1 ? 0 : j, bv.shape(2) == 1 ? 0 : k,
              bv.shape(3) == 1 ? 0 : l) *
            t(i, j, k, l);
  return r;
}

// xtensor<T,4> * broadcast_view<T,4> → xtensor<T,4>
template<typename T>
xtensor<T, 4> operator*(
  const xtensor<T, 4>& t, const broadcast_view<T, 4>& bv)
{
  return bv * t;
}
  ...
  const xtensor<T, 3>& t, const broadcast_view<T, 3>& bv)
{
  ...
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        r(i, j, k) = t(i, j, k) / bv(i, bv.shape(1) == 1 ? 0 : j,
                                     bv.shape(2) == 1 ? 0 : k);
  return r;
}
...
// xtensor<T,3> /= broadcast_view<T,3>
template<typename T>
xtensor<T, 3>& operator/=(xtensor<T, 3>& t, const broadcast_view<T, 3>& bv)
{
  auto s = t.shape();
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        t(i, j, k) /= bv(i, bv.shape(1) == 1 ? 0 : j,
                        bv.shape(2) == 1 ? 0 : k);
  return t;
}

// xtensor<T,4> /= broadcast_view<T,4>
template<typename T>
xtensor<T, 4>& operator/=(xtensor<T, 4>& t, const broadcast_view<T, 4>& bv)
{
  auto s = t.shape();
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
...
// scalar * broadcast_view<T,3> → broadcast_view stores a copy, so return xtensor
template<typename T>
xtensor<T, 3> operator*(T val, const broadcast_view<T, 3>& bv)
{
  auto s = bv.shape();
  xtensor<T, 3> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        r(i, j, k) = val * bv(i, j, k);
  return r;
}

// scalar * broadcast_view<T,4> → xtensor
template<typename T>
xtensor<T, 4> operator*(T val, const broadcast_view<T, 4>& bv)
{
  auto s = bv.shape();
  xtensor<T, 4> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        for (std::size_t l = 0; l < s[3]; ++l)
          r(i, j, k, l) = val * bv(i, j, k, l);
  return r;
}

// xtensor<T,2> * broadcast_view<T,2> → xtensor<T,2>
template<typename T>
xtensor<T, 2> operator*(
  const xtensor<T, 2>& t, const broadcast_view<T, 2>& bv)
{
  auto s = t.shape();
  xtensor<T, 2> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      r(i, j) = t(i, j) * bv(i, bv.shape(1) == 1 ? 0 : j);
  return r;
}

// broadcast_view<T,2> * xtensor<T,2> → xtensor<T,2>
template<typename T>
xtensor<T, 2> operator*(
  const broadcast_view<T, 2>& bv, const xtensor<T, 2>& t)
{
  return t * bv;
}

// xtensor<T,2> /= broadcast_view<T,2>
template<typename T>
xtensor<T, 2>& operator/=(xtensor<T, 2>& t, const broadcast_view<T, 2>& bv)
{
  auto s = t.shape();
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      t(i, j) /= bv(i, bv.shape(1) == 1 ? 0 : j);
  return t;
}
...
// xtensor<T,N>::operator= from broadcast_view (deferred definition)
// Broadcasts the view into the existing tensor shape. Each dimension of the
// broadcast_view is either 1 (broadcast) or matches the target dimension.
// The target tensor shape is NOT changed — broadcasting fills in the values.
template<typename T, std::size_t N>
template<std::size_t BN>
xtensor<T, N>& xtensor<T, N>::operator=(const broadcast_view<T, BN>& bv)
{
  static_assert(N == BN, "broadcast_view dimension must match xtensor dimension");
  auto bvs = bv.shape();
  // Use existing shape, broadcasting singleton dims from the view
  if constexpr (N == 2) {
    for (std::size_t i = 0; i < shape_[0]; ++i)
      for (std::size_t j = 0; j < shape_[1]; ++j)
        (*this)(i, j) = bv(bvs[0] == 1 ? 0 : i, bvs[1] == 1 ? 0 : j);
  } else if constexpr (N == 3) {
    for (std::size_t i = 0; i < shape_[0]; ++i)
      for (std::size_t j = 0; j < shape_[1]; ++j)
        for (std::size_t k = 0; k < shape_[2]; ++k)
          (*this)(i, j, k) = bv(bvs[0] == 1 ? 0 : i,
            bvs[1] == 1 ? 0 : j, bvs[2] == 1 ? 0 : k);
  } else if constexpr (N == 4) {
    for (std::size_t i = 0; i < shape_[0]; ++i)
      for (std::size_t j = 0; j < shape_[1]; ++j)
        for (std::size_t k = 0; k < shape_[2]; ++k)
          for (std::size_t l = 0; l < shape_[3]; ++l)
            (*this)(i, j, k, l) = bv(bvs[0] == 1 ? 0 : i,
              bvs[1] == 1 ? 0 : j, bvs[2] == 1 ? 0 : k,
              bvs[3] == 1 ? 0 : l);
  }
  return *this;
...
// Conversion helper: materialize a broadcast_view into an xtensor
template<typename T>
xtensor<T, 3> eval(const broadcast_view<T, 3>& bv)
{
  auto s = bv.shape();
  xtensor<T, 3> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
...
template<typename T>
xtensor<T, 4> eval(const broadcast_view<T, 4>& bv)
{
  auto s = bv.shape();
  xtensor<T, 4> r(s);
  for (std::size_t i = 0; i < s[0]; ++i)
    for (std::size_t j = 0; j < s[1]; ++j)
      for (std::size_t k = 0; k < s[2]; ++k)
        for (std::size_t l = 0; l < s[3]; ++l)
          r(i, j, k, l) = bv(i, j, k, l);
  return r;
}

//==============================================================================
// Cross-dimension broadcasting: xtensor<T,2> op xtensor<T,1>
// Broadcasts 1D along axis 0 (each row scaled by corresponding 1D element)
//==============================================================================

template<typename T>
xtensor<T, 2> operator*(const xtensor<T, 2>& a, const xtensor<T, 1>& b)
{
  xtensor<T, 2> r(a.shape());
...
template<typename T>
xtensor<T, 2> operator*(const xtensor<T, 1>& b, const xtensor<T, 2>& a)
{
  return a * b;
}

// xtensor<T,3> * xtensor<T,1> → broadcast 1D along axis 0
template<typename T>
xtensor<T, 3> operator*(const xtensor<T, 3>& a, const xtensor<T, 1>& b)
{
  xtensor<T, 3> r(a.shape());
  for (std::size_t i = 0; i < a.shape(0); ++i)
    for (std::size_t j = 0; j < a.shape(1); ++j)
...
template<typename T>
xtensor<T, 3> operator*(const xtensor<T, 1>& b, const xtensor<T, 3>& a)
{
  return a * b;
}

// xtensor<T,3> * xtensor<T,2> → broadcast 2D along axes 0,1
// e.g., [A, D, G] * [A, G] → each (a, d, g) *= (a, g)
template<typename T>
xtensor<T, 3> operator*(const xtensor<T, 3>& a, const xtensor<T, 2>& b)
{
  xtensor<T, 3> r(a.shape());
  // Determine broadcasting: if b.shape matches a's last 2 dims, broadcast along them
  // If b.shape(0) == a.shape(0) and b.shape(1) == a.shape(2), broadcast as (a, _, g)
  if (b.shape(0) == a.shape(0) && b.shape(1) == a.shape(2)) {
    for (std::size_t i = 0; i < a.shape(0); ++i)
      for (std::size_t j = 0; j < a.shape(1); ++j)
        for (std::size_t k = 0; k < a.shape(2); ++k)
          r(i, j, k) = a(i, j, k) * b(i, k);
  } else {
    // Default: broadcast b's dims matching a's first 2 dims
    for (std::size_t i = 0; i < a.shape(0); ++i)
      for (std::size_t j = 0; j < a.shape(1); ++j)
        for (std::size_t k = 0; k < a.shape(2); ++k)
          r(i, j, k) = a(i, j, k) * b(i, j);
  }
  return r;
}

template<typename T>
xtensor<T, 3> operator*(const xtensor<T, 2>& b, const xtensor<T, 3>& a)
{
  return a * b;
}

//==============================================================================
// equal() and filtration() for masked assignment
//==============================================================================

// equal(tensor, val) → bool tensor
template<typename T, std::size_t N>
xtensor<bool, N> equal(const xtensor<T, N>& a, T val)
{
  xtensor<bool, N> r(a.shape());
  for (std::size_t i = 0; i < a.size(); ++i)
...
template<typename T>
xarray<bool> equal(const xarray<T>& a, T val)
{
  xarray<bool> r(a.shape());
  for (std::size_t i = 0; i < a.size(); ++i)
    r.data()[i] = (a.data()[i] == val);
  return r;
}

// filtration_proxy: assigning to it sets elements where mask is true
template<typename T, typename MaskContainer>
class filtration_proxy {
  T* data_;
  const MaskContainer& mask_;
  std::size_t size_;

public:
  filtration_proxy(T* data, const MaskContainer& mask, std::size_t size)
    : data_(data), mask_(mask), size_(size)
  {}

  template<typename U>
  filtration_proxy& operator=(U val)
  {
    for (std::size_t i = 0; i < size_; ++i)
      if (mask_.data()[i])
...
template<typename T, std::size_t N>
filtration_proxy<T, xtensor<bool, N>> filtration(
  xtensor<T, N>& a, const xtensor<bool, N>& mask)
{
  return {a.data(), mask, a.size()};
}

template<typename T>
filtration_proxy<T, xarray<bool>> filtration(
  xarray<T>& a, const xarray<bool>& mask)
{
  return {a.data(), mask, a.size()};
}

//==============================================================================
// range_view_3d: view of xtensor<T,3> sliced on last dimension
//==============================================================================

template<typename T>
class range_view_3d {
  T* data_;
  std::size_t s0_, s1_, s2_full_; // original shape dims
  std::size_t k_start_, k_count_;

public:
  range_view_3d(T* data, std::size_t s0, std::size_t s1, std::size_t s2_full,
    std::size_t k_start, std::size_t k_count)
    : data_(data), s0_(s0), s1_(s1), s2_full_(s2_full), k_start_(k_start),
      k_count_(k_count)
  {}

  // Convert to xtensor<T,3> (copy out)
  operator xtensor<std::remove_const_t<T>, 3>() const
  {
    using U = std::remove_const_t<T>;
    xtensor<U, 3> r({s0_, s1_, k_count_});
    for (std::size_t i = 0; i < s0_; ++i)
      for (std::size_t j = 0; j < s1_; ++j)
        for (std::size_t k = 0; k < k_count_; ++k)
          r(i, j, k) = data_[(i * s1_ + j) * s2_full_ + k_start_ + k];
    return r;
  }

  // Assign from xtensor<U,3> (copy in)
  template<typename U>
  range_view_3d& operator=(const xtensor<U, 3>& src)
  {
    for (std::size_t i = 0; i < s0_; ++i)
      for (std::size_t j = 0; j < s1_; ++j)
        for (std::size_t k = 0; k < k_count_; ++k)
          data_[(i * s1_ + j) * s2_full_ + k_start_ + k] = src(i, j, k);
    return *this;
  }
};

// view(3D, all, all, range) → range_view_3d
template<typename T>
range_view_3d<T> view(
  xtensor<T, 3>& a, xall_type, xall_type, xrange_type r)
{
  auto start = resolve_end(r.start, a.shape(2));
  auto stop = resolve_end(r.stop, a.shape(2));
  return {a.data(), a.shape(0), a.shape(1), a.shape(2), start, stop - start};
}

template<typename T>
range_view_3d<const T> view(
  const xtensor<T, 3>& a, xall_type, xall_type, xrange_type r)
{
  auto start = resolve_end(r.start, a.shape(2));
  auto stop = resolve_end(r.stop, a.shape(2));
  return {a.data(), a.shape(0), a.shape(1), a.shape(2), start, stop - start};
...
//==============================================================================
// reshape_view, transpose, dynamic_view (for weight_windows.cpp)
//==============================================================================

// reshape_view: view of contiguous data with new shape (no copy)
template<typename T>
class reshaped_view {
  const T* data_;
  openmc::vector<std::size_t> shape_;
  openmc::vector<std::size_t> strides_;

public:
  reshaped_view(const T* data, const openmc::vector<std::size_t>& shape)
    : data_(data), shape_(shape)
  {
  ...
  const openmc::vector<std::size_t>& shape() const { return shape_; }
  const openmc::vector<std::size_t>& strides() const { return strides_; }
  const T* data() const { return data_; }
  std::size_t dimension() const { return shape_.size(); }
};

template<typename T, std::size_t N, typename ShapeType>
reshaped_view<T> reshape_view(const xtensor<T, N>& a, const ShapeType& shape)
{
  openmc::vector<std::size_t> s;
  for (auto d : shape)
    s.push_back(static_cast<std::size_t>(d));
...
template<typename T, typename ShapeType>
reshaped_view<T> reshape_view(const xarray<T>& a, const ShapeType& shape)
{
  openmc::vector<std::size_t> s;
  for (auto d : shape)
    s.push_back(static_cast<std::size_t>(d));
...
// transposed_view: permuted-dimension view
template<typename T>
class transposed_view {
  const T* data_;
  openmc::vector<std::size_t> shape_;          // permuted shape
  openmc::vector<std::size_t> orig_strides_;   // original strides permuted

public:
  transposed_view(const T* data, const openmc::vector<std::size_t>& orig_shape,
    const openmc::vector<std::size_t>& orig_strides,
    const openmc::vector<std::size_t>& perm)
    : data_(data)
  {
    auto ndim = perm.size();
    shape_.resize(ndim);
    orig_strides_.resize(ndim);
    for (std::size_t i = 0; i < ndim; ++i) {
      shape_[i] = orig_shape[perm[i]];
      orig_strides_[i] = orig_strides[perm[i]];
    }
  }

  const openmc::vector<std::size_t>& shape() const { return shape_; }
  const T* data() const { return data_; }
  const openmc::vector<std::size_t>& strides() const { return orig_strides_; }
};

template<typename T, typename PermType>
transposed_view<T> transpose(const reshaped_view<T>& rv, const PermType& perm)
{
  openmc::vector<std::size_t> p;
  for (auto d : perm)
    p.push_back(static_cast<std::size_t>(d));
  return {rv.data(), rv.shape(), rv.strides(), p};
}

// dynamic_slice_arg: either an int index or all()
struct dynamic_slice_arg {
  bool is_all;
  int value;

  dynamic_slice_arg(int v) : is_all(false), value(v) {}
  dynamic_slice_arg(xall_type) : is_all(true), value(0) {}
};

// dynamic_view: extract a 2D slice from a transposed_view using a mix of
// integer indices and all() slices.
template<typename T>
xtensor<T, 2> dynamic_view(const transposed_view<T>& tv,
  std::initializer_list<dynamic_slice_arg> args_init)
{
  openmc::vector<dynamic_slice_arg> args(args_init);
  auto& shape = tv.shape();
  auto& strides = tv.strides();

  // Find the two "all" dimensions
  std::size_t all_dim0 = 0, all_dim1 = 0;
  int all_count = 0;
  ...
  for (std::size_t i = 0; i < args.size(); ++i) {
    if (args[i].is_all) {
      ...
      base_offset += static_cast<std::size_t>(args[i].value) * strides[i];
    }
  }

  std::size_t n0 = shape[all_dim0];
  std::size_t n1 = shape[all_dim1];
  std::size_t st0 = strides[all_dim0];
  std::size_t st1 = strides[all_dim1];

  xtensor<T, 2> result({n0, n1});
  for (std::size_t i = 0; i < n0; ++i)
    for (std::size_t j = 0; j < n1; ++j)
      result(i, j) = tv.data()[base_offset + i * st0 + j * st1];
  return result;
}
