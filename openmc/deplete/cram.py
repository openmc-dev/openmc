...
Preserves the develop-branch solver-object API while routing the built-in
CRAM16 and CRAM48 solvers through the C++ IPFCramSolver backend.
"""
...
def _infer_supported_order(alpha, theta, alpha0):
    if (alpha.shape == c16_alpha.shape and np.array_equal(alpha, c16_alpha)
            and np.array_equal(theta, c16_theta) and alpha0 == c16_alpha0):
        return 16
    if (alpha.shape == c48_alpha.shape and np.array_equal(alpha, c48_alpha)
            and np.array_equal(theta, c48_theta) and alpha0 == c48_alpha0):
        return 48
    return None
...
class IPFCramSolver(DepSystemSolver):
    ...
    Provides a :meth:`__call__` compatible with the develop-branch public API.
    The built-in CRAM16 and CRAM48 instances dispatch to the C++ backend. Any
    custom coefficient set falls back to the previous SciPy implementation.
    """
    ...
    def __init__(self, alpha, theta, alpha0):
        ...
        self.theta = theta
        self.alpha0 = alpha0
        self._order = _infer_supported_order(alpha, theta, alpha0)
    ...
    def __call__(self, A, n0, dt, substeps=1):
        ...
        if self._order is not None:
            return _cram_solve(A, n0, dt, order=self._order, substeps=substeps)
