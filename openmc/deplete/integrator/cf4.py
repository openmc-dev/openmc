...
def cf4(operator, print_out=True):
    """ Performs integration of an operator using the CF4 algorithm.
    ...
    for i, dt in enumerate(operator.settings.dt_vec):
        ...
        for mat in range(n_mats):
            ...
            x_new = CRAM48(-1/2 * f0 + f2, x[1][mat], dt)
        ...
        for mat in range(n_mats):
            ...
            x_new = CRAM48(1/4*f0 + 1/6*f1 + 1/6*f2 - 1/12*f3, x_new, dt)
            x_new = CRAM48(-1/12*f0 + 1/6*f1 + 1/6*f2 + 1/4*f3, x_new, dt)
