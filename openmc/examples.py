...
def pwr_core() -> openmc.Model:
    ...
    tube_cold = openmc.Universe(
        name="Instrumentation guide tube, " "cold water", universe_id=2
    )
    ...
    tube_hot = openmc.Universe(
        name="Instrumentation guide tube, hot water", universe_id=4
    )
...
def pwr_assembly() -> openmc.Model:
    ...
    gt_inner_cell = openmc.Cell(
        name="guide tube inner water", fill=hot_water, region=-fuel_or
    )
    gt_clad_cell = openmc.Cell(
        name="guide tube clad", fill=clad, region=+fuel_or & -clad_or
    )
    gt_outer_cell = openmc.Cell(
        name="guide tube outer water", fill=hot_water, region=+clad_or
    )
...
def slab_mg(num_regions=1, mat_names=None, mgxslib_name="2g.h5") -> openmc.Model:
    ...
    rads = np.linspace(0.0, rad_outer, len(mats) + 1, endpoint=True)[1:]
    ...
    surfs = []
    surfs.append(openmc.XPlane(x0=0.0, boundary_type="reflective"))
    ...
    INF = 1000.0
    bounds = [0.0, -INF, -INF, rads[0], INF, INF]
...
def random_ray_lattice() -> openmc.Model:
    ...
    h2o_xsdata.order = 0
    h2o_xsdata.set_total(
        [0.15920605, 0.412969593, 0.59030986, 0.58435, 0.718, 1.2544497, 2.650379]
    )
    h2o_xsdata.set_absorption(
        [
            6.0105e-04,
            1.5793e-05,
            3.3716e-04,
    ...
    fuel_b = openmc.Cell(
        fill=uo2, region=+inner_ring_a & -inner_ring_b, name="fuel inner b"
    )
    ...
    lattice2x2.universes = [[pincell, pincell], [pincell, mod_lattice_uni]]
...
def random_ray_three_region_cube() -> openmc.Model:
    ...
    full_domain = openmc.Cell(
        fill=lattice_uni,
        region=+x_low & -x_high & +y_low & -y_high & +z_low & -z_high,
        name="full domain",
    )
    ...
    source = openmc.IndependentSource(
        ...
        constraints={"domains": [source_universe]},
        strength=3.14,
    )
