        ...
        if not isinstance(subdomains, basestring):
            ...
            if (len(self.domain.dimension) == 3):
                ...
                for x in range(1,nx+1):
                    for y in range(1,ny+1):
                        for z in range(1,nz+1):
                            subdomains_list.append((x, y, z))
            else:
                nx, ny = self.domain.dimension
                for x in range(1,nx+1):
                    for y in range(1,ny+1):
                        subdomains_list.append((x, y, 1))
            subdomains = [tuple(subdomains_list)]
        else:
...
class ChiDelayed(Chi):
    ...
    :attr:`ChiDelayed.domain` properties. Tallies for the flux and appropriate reaction
    rates over the specified domain are generated automatically via the
    ...
    def xs_tally(self):
        ...
        if self._xs_tally is None:
            nu_fission_in = self.tallies['delayed-nu-fission-in']
    ...
    def get_slice(self, nuclides=[], groups=[]):
        ...
        if len(groups) != 0:
            ...
            # Slice nu-fission-out tally along energyout filter
            nu_fission_out = slice_xs.tallies['delayed-nu-fission-out']
            tally_slice = nu_fission_out.get_slice(filters=['energyout'],
                                                   filter_bins=filter_bins)
    ...
    def get_xs(self, groups='all', subdomains='all', nuclides='all',
        ...
        if self.by_nuclide:
            ...
            if nuclides == 'sum' or nuclides == ['sum']:
                ...
                # Retrieve the fission production tallies
                nu_fission_in = self.tallies['delayed-nu-fission-in']
                nu_fission_out = self.tallies['delayed-nu-fission-out']
