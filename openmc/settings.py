...
class Settings:
    ...
    **kwargs : dict, optional
    ...
    surf_source_write : dict
        ...
        :info_surface_source: write message with surface source info (bool)
    ...
    def surf_source_write(self, surf_source_write: dict):
        ...
        for key, value in surf_source_write.items():
            cv.check_value(
                ...
                key,
                ("surface_ids", "max_particles", "max_files", "info_surface_source", "mcpl", "cell", "cellfrom", "cellto"),
            )
            if key == "surface_ids":
                cv.check_type(
    ...
    def _surf_source_write_from_xml_element(self, root):
        ...
        if elem is None:
            return
        for key in ('surface_ids', 'max_particles', 'max_files', 'info_surface_source', 'mcpl', 'cell', 'cellto', 'cellfrom'):
            value = get_text(elem, key)
            if value is not None:
                if key == 'surface_ids':
                    value = [int(x) for x in value.split()]
