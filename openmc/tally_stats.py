...
def _calc_b1_b2(n, mean, sum_sq, sum_rd, sum_th):
    """
    Compute unbiased estimators sample skewness and kurtosis based on the tallies data.
    """
    m2 = sum_sq / n - mean**2
    ...
    sqrt_b1 = (np.sqrt(n * (n - 1)) / (n - 2)) * np.where(
        m2 > 0, m3 / (m2 ** (1.5)), 0.0
    )

    # Unbiased sample kurtosis
    b2 = (((n + 1) * n) / ((n - 1) * (n - 2) * (n - 3))) * np.where(
        m2 > 0, m4 / (m2**2), 0.0
    ) - 3 * (n - 1) * (n - 1) / ((n - 2) * (n - 3))
    return sqrt_b1, b2
...
def kurtosis_test(n, mean, sum_sq, sum_rd, sum_th, alternative="two-sided"):
    ...
    moment = ((6.0 * (n**2 - 5.0 * n + 2.0)) / ((n + 7.0) * (n + 9.0))) * np.sqrt(
    ...
    Zb2 = (
        1.0
        - 2.0 / (9.0 * A)
        - ((1.0 - 2.0 / A) / (1.0 + x * np.sqrt(2.0 / (A - 4.0)))) ** (1.0 / 3.0)
    ) / np.sqrt(2.0 / (9.0 * A))
...
def print_normality_tests_summary(
    n,
    mean,
    sum_sq,
    sum_rd,
