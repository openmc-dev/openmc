...
void print_config_usage()
{
  ...
  std::cout << "  -p, --prefix         Print the installation prefix"
            << std::endl;
  std::cout << "  -i, --include-dir    Print the include directory"
            << std::endl;
  std::cout << "  -l, --lib-dir        Print the library directory"
            << std::endl;
  std::cout << "  -c, --cmake-dir      Print the CMake configuration directory"
            << std::endl;
  std::cout
    << "  -L, --libs           Print the all libraries in the library directory"
    << std::endl;
  std::cout << "  -e, --extra-lib-dir  Print the extra library directory"
            << std::endl;
...
int openmc_config_main(int argc, char** argv)
{
  ...
  try {
    ...
    include_dir_val = exec("python3 -c \"import openmc.paths; "
                           "print(''.join(openmc.paths.include_path))\"");
    lib_dir_val = exec("python3 -c \"import openmc.paths; "
                       "print(''.join(openmc.paths.lib_path))\"");
    cmake_dir_val = exec(
      "python3 -c \"import openmc.paths; print(openmc.paths.cmake_path)\" ");
    libs_val = exec(
      "python3 -c \"import openmc.paths; print(' '.join(openmc.paths.lib))\"");
    extra_libs_val = exec("python3 -c \"import openmc.paths; print(' "
                          "'.join(openmc.paths.extra_lib))\"");
    extra_lib_dir_val = exec(
      "python3 -c \"import openmc.paths; print(openmc.paths.extra_lib_path)\"");
  } catch (const std::runtime_error& e) {
    std::cerr << "Fatal error: Could not get configuration from openmc.paths "
                 "via Python: "
              << e.what() << std::endl;
    return 1;
  }
  ...
  include_dir_val =
    std::string(CMAKE_INSTALL_PREFIX) + "/" + CMAKE_INSTALL_INCLUDEDIR;
  lib_dir_val = std::string(CMAKE_INSTALL_PREFIX) + "/" + CMAKE_INSTALL_LIBDIR;
  cmake_dir_val = std::string(CMAKE_INSTALL_PREFIX) + "/" +
                  CMAKE_INSTALL_LIBDIR + "/cmake/OpenMC";
