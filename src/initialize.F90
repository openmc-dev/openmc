...
!===============================================================================
  ...
  subroutine initialize_mpi()
    ...
    type(Bank)       :: b
    type(TallyResult) :: tr
    integer                   :: part_blocks(14) ! ParticleBuffer counts
    integer                   :: part_types(14)  ! ParticleBuffer datatypes
    integer(MPI_ADDRESS_KIND) :: part_disp(14)   ! ParticleBuffer displacements
    ...
    call MPI_GET_ADDRESS(ptmp % stored_fly_dist,  part_disp(5), mpi_err)
    call MPI_GET_ADDRESS(ptmp % tot_domain_dist,  part_disp(6), mpi_err)
    call MPI_GET_ADDRESS(ptmp % last_xyz,         part_disp(7), mpi_err)
    call MPI_GET_ADDRESS(ptmp % last_xyz_current, part_disp(8), mpi_err)
    call MPI_GET_ADDRESS(ptmp % stored_xyz,       part_disp(9), mpi_err)
    call MPI_GET_ADDRESS(ptmp % stored_uvw,       part_disp(10), mpi_err)
    call MPI_GET_ADDRESS(ptmp % type,             part_disp(11), mpi_err)
    call MPI_GET_ADDRESS(ptmp % surface,          part_disp(12), mpi_err)
    call MPI_GET_ADDRESS(ptmp % cell_born,        part_disp(13), mpi_err)
    call MPI_GET_ADDRESS(ptmp % n_collision,      part_disp(14), mpi_err)

    ! Adjust displacements
    part_disp = part_disp - part_disp(1)

    ! Define particle type
    part_blocks = (/ 1, N_STREAMS, 1, 1, 1, 1, 3, 3, 3, 3, 1, 1, 1, 1 /)
    part_types = (/ MPI_INTEGER8, MPI_INTEGER8, MPI_REAL8, MPI_REAL8, &
         MPI_REAL8, MPI_REAL8, MPI_REAL8, MPI_REAL8, MPI_REAL8, MPI_REAL8, &
         MPI_INTEGER, MPI_INTEGER, MPI_INTEGER, MPI_INTEGER /)
    call MPI_TYPE_CREATE_STRUCT(14, part_blocks, part_disp, part_types, &
