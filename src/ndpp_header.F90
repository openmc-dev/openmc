...
!===============================================================================
  ...
  type Ndpp
    real(8), allocatable :: el_Ein(:)           ! Incoming elastic energy grid
    integer, allocatable :: el_Ein_srch(:)      ! Incoming elastic energy grid search bounds
    type(GrpTransfer), allocatable :: el(:)     ! Elastic Data, Dimension is # of Ein
    real(8), allocatable :: inel_Ein(:)         ! Incoming inelastic energy grid
    integer, allocatable :: inel_Ein_srch(:)    ! Incoming inelastic energy grid search bounds
    type(GrpTransfer), allocatable :: inel(:)   ! Inelastic Data, Dimension is # of Ein
    type(GrpTransfer), allocatable :: nuinel(:) ! Inelastic Data, Dimension is # of Ein
    ...
    logical, intent(in)  :: is_init = .false.   ! Is object initialized

    ! Type-Bound procedures
    contains
      procedure :: init => ndpp_init ! Initialized NDPP structure
      procedure :: clear => ndpp_clear ! Clears NDPP structure
      procedure :: tally_scatt_n => ndpp_tally_scatt_n ! Tallies NDPP-scatter-n
      procedure :: tally_scatt_pn => ndpp_tally_scatt_pn ! Tallies NDPP-scatter-pn
  ...
  subroutine ndpp_init(this, listing, get_scatt, get_nuscatt, get_chi_t, get_chi_p,
    ...
    if (is_nuc) then
      if (.not. get_scatt) then
        if (allocated(this % inel)) then
          deallocate(this % inel)
        end if
      end if
      if (.not. get_nuscatt) then
        if (allocated(this % nuinel)) then
      ...
      if ((.not. get_scatt) .and. (.not. get_nuscatt)) then
        ...
        if (allocated(this % inel_Ein)) then
          deallocate(this % inel_Ein)
        end if
        if (allocated(this % inel_Ein_srch)) then
...
!===============================================================================
  ...
  subroutine ndpp_tally_n(this, i_nuclide, gin, score_index, filter_index, t_order, &
...
!===============================================================================
  ...
  subroutine ndpp_tally_pn(this, i_nuclide, gin, score_index, filter_index, t_order, &
    mult, is_analog, Ein, results, nuscatt)
