...
!===============================================================================
  ...
  subroutine write_particle_restart()
    ...
    filename = trim(path_output) // 'particle_' // trim(to_str(current_batch)) &
         // '_' // trim(to_str(current_work)) // '_' // trim(to_str(rank)) &
         // '.h5'
#else
    filename = trim(path_output) // 'particle_' // trim(to_str(current_batch)) &
         // '_' // trim(to_str(current_work)) // '_' // trim(to_str(rank)) &
    ...
    ! Create hdf5 file
    call file_create(filename, 'serial', rank)

    ! Get information about source particle
    src => source_bank(current_work)

    ! Write data to file
    call write_data(FILETYPE_PARTICLE_RESTART, 'filetype') 
    call write_data(REVISION_PARTICLE_RESTART, 'revision')
    call write_data(current_batch, 'current_batch')
    call write_data(gen_per_batch, 'gen_per_batch')
    call write_data(current_gen, 'current_gen')
    call write_data(n_particles, 'n_particles')
    call write_data(p % id, 'id')
    call write_data(src % wgt, 'weight')
    call write_data(src % E, 'energy')
    call write_data(src % xyz, 'xyz', length = 3) 
    call write_data(src % uvw, 'uvw', length = 3)

    ! Close hdf5 file
    call file_close('serial', rank)
