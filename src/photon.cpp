...
PhotonInteraction::PhotonInteraction(hid_t group)
{
  ...
  for (int i = 0; i < n_shell; ++i) {
    ...
    auto cross_section =
      cross_sections_.col(i).slice(static_cast<size_t>(shell.threshold));
    ...
    if (object_exists(tgroup, "transitions")) {
      ...
      if (n_transition > 0) {
        ...
        double norm = tensor::Tensor<double>(matrix.col(3)).sum();
  ...
  if (settings::electron_treatment == ElectronTreatment::TTB) {
    ...
    if (cutoff > E(0)) {
      ...
      frst(0) = cutoff;
      tensor::Tensor<double> rest(electron_energy.slice(i_grid + 1));
