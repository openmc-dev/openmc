...
void collision_mg(Particle& p)
{
  ...
  void create_fission_sites(Particle & p)
  {
    ...
    for (n_sites_stored = 0; n_sites_stored < nu; n_sites_stored++) {
      ...
      int dg;
      int gout;
      data::mg.macro_xs_[p.material()].sample_fission_energy(p.g(), dg, gout,
        p.current_seed(), p.mg_xs_cache().t, p.mg_xs_cache().a);
  ...
  void absorption(Particle & p)
  {
    if (settings::survival_biasing) {
      ...
      double wgt_absorb =
        p.wgt() * p.macro_xs().absorption / p.macro_xs().total;
      ...
      if (p.macro_xs().absorption >
          prn(p.current_seed()) * p.macro_xs().total) {
        p.keff_tally_absorption() +=
