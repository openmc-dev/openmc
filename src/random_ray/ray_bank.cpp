...
void RayBank::buffer_ray_data_to_send(RandomRay& ray, FlatSourceDomain* domain)
{
  ...
  rd.n_transfers = rbc.n_transfers;
  ...
  std::memcpy(
    rd.handles, rbc.handles, MAX_N_HANDLES * sizeof(moab::EntityHandle));
#endif
  ...
  buffers.angular_flux.insert(buffers.angular_flux.end(),
    rbc.angular_flux.begin(), rbc.angular_flux.end());

  // Pack coord and cell_last arrays directly
  buffers.coord.insert(buffers.coord.end(), rbc.coord.begin(), rbc.coord.end());
  buffers.cell_last.insert(
    buffers.cell_last.end(), rbc.cell_last.begin(), rbc.cell_last.end());
...
void RayBank::update(FlatSourceDomain* domain)
{
  ...
  // Empty ray list because rays have either died or are in buffer to be sent to
  // other ranks
...
// Tells each rank how many rays to receive from who and how many rays to send
// to who
void RayBank::communicate_message_metadata()
{
  ...
  MPI_Alltoall(num_messages_sending_.data(), 1, MPI_INT,
    num_messages_receiving_.data(), 1, MPI_INT, mpi::intracomm);

  total_receiving_rays_ = accumulate(
    num_messages_receiving_.begin(), num_messages_receiving_.end(), 0);
}
...
void RayBank::communicate_rays()
{
  ...
  for (int i = 0; i < mpi::n_procs; i++) {
    if (num_messages_receiving_[i] > 0)
      num_recv_ranks++;
  }
  ...
  for (auto& [receiving_rank, buffers] : ray_send_buffer_) {
    ...
    for (int i = 0; i < num_rays; i++) {
      if (buffers.ray_data[i].distance_travelled > 0.0 ||
          buffers.ray_data[i].is_active) {
    ...
    MPI_Isend(buffers.ray_data.data(), num_rays * sizeof(RayExchangeData),
      MPI_BYTE, receiving_rank, 1, mpi::intracomm, &requests[req_idx++]);

    MPI_Isend(buffers.angular_flux.data(), num_rays * negroups_, MPI_FLOAT,
      receiving_rank, 2, mpi::intracomm, &requests[req_idx++]);

    MPI_Isend(buffers.coord.data(), num_rays * n_coord_max * sizeof(LocalCoord),
      MPI_BYTE, receiving_rank, 3, mpi::intracomm, &requests[req_idx++]);

    MPI_Isend(buffers.cell_last.data(), num_rays * n_coord_max, MPI_INT,
      receiving_rank, 4, mpi::intracomm, &requests[req_idx++]);
  }
...
#pragma omp parallel for
  for (int64_t i = 0; i < received_ray_data_.size(); i++) {
    ...
    my_ray_list_[i].restart_ray(domain, received_ray_data_[i],
      ...
      &received_coord_[i * n_coord_max], &received_cell_last_[i * n_coord_max]);
  }
