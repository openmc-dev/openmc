...
{
  if (p.type() == PDG_PHOTON && p.event_mt() == PAIR_PROD) {
    // pair production
    return -2 * MASS_ELECTRON_EV;
  } else if (p.type() == PDG_POSITRON) {
...
#pragma omp atomic
    ...
    case SCORE_IFP_TIME_NUM:
      if (settings::ifp_on) {
        if ((p.type() == PDG_NEUTRON) && (p.fission())) {
...
void score_analog_tally_ce(Particle& p)
{
  ...
  // electrons/positrons.
  double flux = (p.type() == PDG_NEUTRON || p.type() == PDG_PHOTON) ? 1.0 : 0.0;
