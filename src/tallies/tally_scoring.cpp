...
{
  if (p.type().pdg_number() == PDG_PHOTON && p.event_mt() == PAIR_PROD) {
    // pair production
...
#pragma omp atomic
    ...
    case SCORE_IFP_TIME_NUM:
      if (settings::ifp_on) {
        if ((p.type().pdg_number() == PDG_NEUTRON) && (p.fission())) {
    ...
    case PHOTOELECTRIC:
    case PAIR_PROD:
      if (p.type().pdg_number() != PDG_PHOTON)
        continue;
...
void score_analog_tally_ce(Particle& p)
{
  ...
  // electrons/positrons.
  double flux = (p.type().pdg_number() == PDG_NEUTRON ||
                  p.type().pdg_number() == PDG_PHOTON)
                  ? 1.0
                  : 0.0;
