...
FunctionForTPMS::FunctionForTPMS(double _xmin, double _xmax, double _ymin, double _ymax, double _zmin, double _zmax)
...
void interpolate_1d(double coord, const std::vector<double>& grid_coords, int& lower_index, int& upper_index, double& lower_weight, double& upper_weight) 
{
    int num_points = grid_coords.size();

    auto it = std::upper_bound(grid_coords.begin(), grid_coords.end(), coord);
    upper_index = std::min(static_cast<int>(std::distance(grid_coords.begin(), it)), num_points - 1);
...
InterpolationForTPMS::InterpolationForTPMS(std::vector<double> _x_grid, std::vector<double> _y_grid, std::vector<double> _z_grid, std::vector<std::vector<std::vector<double>>> _matrix)
 : FunctionForTPMS(_x_grid[0], _x_grid[_x_grid.size() - 1], _y_grid[0], _y_grid[_y_grid.size() - 1], _z_grid[0], _z_grid[_z_grid.size() - 1]),
   x_grid(_x_grid),
   y_grid(_y_grid),
   z_grid(_z_grid),
   matrix(_matrix)
