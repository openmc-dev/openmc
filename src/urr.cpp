...
UrrData::UrrData(hid_t group_id)
{
  ...
  xt::xtensor<double, 2>::shape_type new_shape_orig = {n_energy, n_cdf_values};

  // xtensor's size_type does not necessary match tensor our size_type
  std::array<decltype(cdf_values_)::size_type, 2> new_shape;
  new_shape[0] = new_shape_orig[0];
  new_shape[1] = new_shape_orig[1];
