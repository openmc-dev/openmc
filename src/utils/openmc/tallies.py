...
class Filter(object):
  ...
  def setBinEdges(self, bins):
    ...
    if self._type in ['cell', 'cellborn', 'surface', 'material',
      ...
      for edge in bins:
        ...
        if not is_integer(edge):
          msg = 'Unable to add bin edge {0} to a {1} Filter since it ' \
                'is a non-integer'.format(edge, self._type)
          raise ValueError(msg)

        elif edge < 0:
          msg = 'Unable to add bin edge {0} to a {1} Filter since it ' \
                'is a negative integer'.format(edge, self._type)
          raise ValueError(msg)
      ...
      for edge in bins:
        ...
        if not is_integer(edge) and not is_float(edge):
          msg = 'Unable to add bin edge {0} to {1} Filter since it is a ' \
                'non-integer or floating point value'.format(edge, self._type)
          raise ValueError(msg)

        elif edge < 0.:
          msg = 'Unable to add bin edge {0} to {1} Filter since it is ' \
                'a negative value'.format(edge, self._type)
          raise ValueError(msg)
      ...
      for index in range(len(bins)):
        ...
        if index > 0 and bins[index] < bins[index-1]:
          msg = 'Unable to add bin edges {0} to {1} Filter since they are ' \
                'not monotonically increasing'.format(bins, self._type)
          raise ValueError(msg)
    ...
    # mesh filters
      ...
      if not len(bins) == 1:
        msg = 'Unable to add bin edges {0} to a mesh Filter since only a ' \
              'single mesh can be used per tally'.format(bins)
        raise ValueError(msg)
  ...
  def setNumBins(self, num_bins):
    ...
    if not is_integer(num_bins) or num_bins < 0:
      msg = 'Unable to set the number of bins {0} for a {1} Filter since ' \
            'it is not a positive integer'.format(num_bins, self._type)
      raise ValueError(msg)
...
class Mesh(object):
  ...
  def setDimension(self, dimension):
    ...
    if not isinstance(dimension, (tuple, list, np.ndarray)):
      msg = 'Unable to set Mesh ID={0} with dimension {1} which is not ' \
            'a Python list, tuple or NumPy array'.format(self._id, dimension)
      raise ValueError(msg)

    elif len(dimension) != 2 and len(dimension) != 3:
      msg = 'Unable to set Mesh ID={0} with dimension {1} since it must ' \
            'include 2 or 3 dimensions'.format(self._id, dimension)
      raise ValueError(msg)

    for dim in dimension:

      if not is_integer(dim):
        msg = 'Unable to set Mesh ID={0} with dimension {1} which is a ' \
              'non-integer'.format(self._id, dim)
        raise ValueError(msg)
  ...
  def setLowerLeft(self, lower_left):
    ...
    if not isinstance(lower_left, (tuple, list, np.ndarray)):
      msg = 'Unable to set Mesh ID={0} with lower_left {1} which is not a ' \
            'Python list, tuple or NumPy array'.format(self._id, lower_left)
      raise ValueError(msg)

    elif len(lower_left) != 2 and len(lower_left) != 3:
      msg = 'Unable to set Mesh ID={0} with lower_left {1} since it must ' \
            'include 2 or 3 dimensions'.format(self._id, lower_left)
      raise ValueError(msg)
  ...
  def setUpperRight(self, upper_right):
    ...
    if not isinstance(upper_right, (tuple, list, np.ndarray)):
      msg = 'Unable to set Mesh ID={0} with upper_right {1} which is not a ' \
            'Python list, tuple or NumPy array'.format(self._id, upper_right)
      raise ValueError(msg)

    if len(upper_right) != 2 and len(upper_right) != 3:
      msg = 'Unable to set Mesh ID={0} with upper_right {1} since it must ' \
            'include 2 or 3 dimensions'.format(self._id, upper_right)
      raise ValueError(msg)

    for coord in upper_right:

      if not is_integer(coord) and not is_float(coord):
        msg = 'Unable to set Mesh ID={0} with upper_right {1} which is ' \
  ...
  def setWidth(self, width):
    ...
    if not width is None:
      ...
      if not isinstance(width, (tuple, list, np.ndarray)):
        msg = 'Unable to set Mesh ID={0} with width {1} which is not a ' \
              'Python list, tuple or NumPy array'.format(self._id, width)
        raise ValueError(msg)
    ...
    for dim in width:
      ...
      if not is_integer(dim) and not is_float(dim):
        msg = 'Unable to set Mesh ID={0} with width {1} which is neither ' \
              'an integer nor a floating point value'.format(self._id, width)
        raise ValueError(msg)
...
class Tally(object):
  ...
  def setEstimator(self, estimator):
    if not estimator in ['analog', 'tracklength']:
      msg = 'Unable to set the estimator for Tally ID={0} to {1} since it ' \
            'is not a valid estimator type'.format(self._id, estimator)
      raise ValueError(msg)
  ...
  def setNumRealizations(self, num_realizations):
    ...
    if not is_integer(num_realizations):
      msg = 'Unable to set the number of realizations to {0} for Tally ID={1} ' \
            'since it is not an integer'.format(num_realizations)
      raise ValueError(msg)

    elif num_realizations < 0:
      msg = 'Unable to set the number of realizations to {0} for Tally ID={1} ' \
            'since it is a negative value'.format(num_realizations)
      raise ValueError(msg)
  ...
  def setResults(self, sum, sum_sq):
    ...
    if not isinstance(sum, (tuple, list, np.ndarray)):
      msg = 'Unable to set the sum to {0}for Tally ID={1} since it is not ' \
            'a Python tuple/list or NumPy array'.format(sum, self._id)
      raise ValueError(msg)

    if not isinstance(sum_sq, (tuple, list, np.ndarray)):
      msg = 'Unable to set the sum to {0}for Tally ID={1} since it is not ' \
            'a Python tuple/list or NumPy array'.format(sum_sq, self._id)
      raise ValueError(msg)
  ...
  def removeScore(self, score):
    ...
    if not score in self._scores:
      msg = 'Unable to remove score {0} from Tally ID={1} since the Tally ' \
            'does not contain this score'.format(score, self._id)
      ValueError(msg)
  ...
  def removeFilter(self, filter):
    ...
    if not filter in self._filters:
      msg = 'Unable to remove filter {0} from Tally ID={1} since the Tally ' \
            'does not contain this filter'.format(filter, self._id)
      ValueError(msg)
  ...
  def removeNuclide(self, nuclide):
    ...
    if not nuclide in self._nuclides:
      msg = 'Unable to remove nuclide {0} from Tally ID={1} since the Tally ' \
            'does not contain this nuclide'.format(nuclide, self._id)
      ValueError(msg)
  ...
  def computeStdDev(self, t_value=1.0):
    ...
    self._std_dev = t_value * np.sqrt((self._sum_sq / self._num_realizations - \
  ...
  def getTallyXML(self):
    ...
    if len(self._scores) == 0:
      msg = 'Unable to get XML for Tally ID={0} since it does not contain ' \
            'any scores'.format(self._id)
      raise ValueError(msg)
  ...
  def findFilter(self, filter_type, bins):
    ...
    # Otherwise, throw an Exception
      ...
      msg = 'Unable to find filter type {0} with bin edges {1} in Tally ' \
            'ID={2}'.format(filter_type, bins, self._id)
      raise ValueError(msg)
  ...
  def getValue(self, score, filters, filter_bins, nuclide, value='mean'):
    ...
    for i, filter in enumerate(filters):
      ...
      # Filter bins for a distribcell are the "IDs" of each unique placement
    ...
    if value == 'mean':
      ...
      msg = 'Unable to return results from Tally ID={0} for score {1} ' \
            'since the value {2} is not \'mean\', \'std_dev\', \'sum\', ' \
            'or \'sum_sq\''.format(self._id, score, value)
  ...
  def exportResults(self, filename='tally-results', directory='.',
    ...
    if not is_string(filename):
      msg = 'Unable to export the results for Tally ID={0} to filename={1} ' \
            'since it is not a string'.format(self._id, filename)
      raise ValueError(msg)

    elif not is_string(directory):
      msg = 'Unable to export the results for Tally ID={0} to directory={1} ' \
            'since it is not a string'.format(self._id, directory)
      raise ValueError(msg)

    elif not format in ['hdf5', 'pkl', 'csv']:
      msg = 'Unable to export the results for Tally ID={0} to format {1} ' \
            'since it is not supported'.format(self._id, format)
      raise ValueError(msg)

    elif not isinstance(append, (bool, np.bool)):
      msg = 'Unable to export the results for Tally ID={0} since the append ' \
            'parameters is not True/False'.format(self._id, append)
      raise ValueError(msg)
