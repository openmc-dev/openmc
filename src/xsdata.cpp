  ...
  n_dg_(n_d_groups)
{
  ...
  total = xt::zeros<xsfloat>(shape);
  absorption = xt::zeros<xsfloat>(shape);
  inverse_velocity = xt::zeros<xsfloat>(shape);
  if (fissionable) {
    fission = xt::zeros<xsfloat>(shape);
    nu_fission = xt::zeros<xsfloat>(shape);
    prompt_nu_fission = xt::zeros<xsfloat>(shape);
    kappa_fission = xt::zeros<xsfloat>(shape);
  }

  // allocate decay_rate; [temperature][angle][delayed group]
  shape[1] = n_dg_;
  decay_rate = xt::zeros<xsfloat>(shape);

  if (fissionable) {
    shape = {n_ang, n_dg_, n_g_};
    // allocate delayed_nu_fission; [temperature][angle][delay group][in group]
    delayed_nu_fission = xt::zeros<xsfloat>(shape);

    // chi_prompt; [temperature][angle][in group][out group]
    shape = {n_ang, n_g_, n_g_};
    chi_prompt = xt::zeros<xsfloat>(shape);

    // chi_delayed; [temperature][angle][delay group][in group][out group]
    shape = {n_ang, n_dg_, n_g_, n_g_};
    chi_delayed = xt::zeros<xsfloat>(shape);
  }
  ...
  bool is_isotropic)
{
  ...
  // Get chi
  xt::xtensor<xsfloat, 2> temp_chi({n_ang, n_g_}, 0.);
  ...
  // Get nu-fission
  xt::xtensor<xsfloat, 2> temp_nufiss({n_ang, n_g_}, 0.);
  ...
  if (beta_ndims == ndim_target) {
    xt::xtensor<xsfloat, 2> temp_beta({n_ang, n_dg_}, 0.);
    ...
    xt::xtensor<xsfloat, 3> temp_beta({n_ang, n_dg_, n_g_}, 0.);
    read_nd_vector(xsdata_grp, "beta", temp_beta, true);
...
XsData::fission_vector_no_beta_from_hdf5(hid_t xsdata_grp, size_t n_ang)
{
  ...
  // Get chi-prompt
  xt::xtensor<xsfloat, 2> temp_chi_p({n_ang, n_g_}, 0.);
  ...
  // Get chi-delayed
  xt::xtensor<xsfloat, 3> temp_chi_d({n_ang, n_dg_, n_g_}, 0.);
...
XsData::fission_matrix_beta_from_hdf5(hid_t xsdata_grp, size_t n_ang, bool is_isotropic)
{
  ...
  xt::xtensor<xsfloat, 3> temp_matrix({n_ang, n_g_, n_g_}, 0.);
  ...
  if (beta_ndims == ndim_target) {
    ...
    xt::xtensor<xsfloat, 1> temp_beta_sum({n_ang}, 0.);
    ...
    xt::xtensor<xsfloat, 2> temp_beta_sum({n_ang, n_g_}, 0.);
    temp_beta_sum = xt::sum(temp_beta, {1});
...
XsData::fission_matrix_no_beta_from_hdf5(hid_t xsdata_grp, size_t n_ang)
{
  ...
  xt::xtensor<xsfloat, 3> temp_matrix_p({n_ang, n_g_, n_g_}, 0.);
  ...
  xt::xtensor<xsfloat, 4> temp_matrix_d({n_ang, n_dg_, n_g_, n_g_}, 0.);
    ...
    AngleDistributionType final_scatter_format, int order_data)
{
  ...
  xsfloat_4dvec input_scatt(n_ang, xsfloat_3dvec(n_g_));
  xt::xtensor<xsfloat, 1> temp_arr({length}, 0.);
  ...
  xsfloat_3dvec temp_mult(n_ang, xsfloat_2dvec(n_g_));
