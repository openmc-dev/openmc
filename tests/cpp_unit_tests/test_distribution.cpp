...
TEST_CASE("Test analytic vs stochastic integral ")
{
  uint64_t seed = openmc::init_seed(0, 0);
  ...
  REQUIRE_THAT(discrete.integral(-2.0, 1.0),
    Catch::Matchers::WithinRel(
      discrete.Distribution::integral(-2.0, 1.0), 1e-2));

  openmc::Uniform uniform(-1.0, 1.0);
  REQUIRE_THAT(uniform.integral(-1.0, 0.5),
    Catch::Matchers::WithinRel(
      uniform.Distribution::integral(-1.0, 0.5), 1e-2));

  double n[4] = {0.0, -1.5, -2.0, 3.0};
  for (auto i : n) {
    openmc::PowerLaw powerlaw(1.0, 5.0, i);
    REQUIRE_THAT(powerlaw.integral(1.0, 2.0),
      Catch::Matchers::WithinRel(
        powerlaw.Distribution::integral(1.0, 2.0), 1e-2));
  }

  openmc::Maxwell maxwell(1.0);
  REQUIRE_THAT(maxwell.integral(0.0, 1.0),
    Catch::Matchers::WithinRel(maxwell.Distribution::integral(0.0, 1.0), 1e-2));

  openmc::Watt watt(0.5, 3.0);
  REQUIRE_THAT(watt.integral(0.0, 1.0),
    Catch::Matchers::WithinRel(watt.Distribution::integral(0.0, 1.0), 1e-2));

  openmc::Normal normal(0.5, 3.0);
  REQUIRE_THAT(normal.integral(-openmc::INFTY, 0.0),
    Catch::Matchers::WithinRel(
      normal.Distribution::integral(-openmc::INFTY, 0.0), 1e-2));
}
