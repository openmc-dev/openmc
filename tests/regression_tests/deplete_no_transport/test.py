...
@pytest.mark.parametrize(
    "multiproc, from_nuclides, normalization_mode, power, source_rate",
    [
...
def test_against_self(
    ...
    op = _create_operator(
        from_nuclides, fuel, flux, micro_xs, chain_file, normalization_mode
    )
    ...
    openmc.deplete.PredictorIntegrator(
        op, dt, power=power, source_rates=source_rate, timestep_units="s"
    ).integrate()
...
@pytest.mark.parametrize(
    "multiproc, dt, time_units, time_type, atom_tol, rx_tol ",
    [
...
def test_against_coupled(
    ...
    openmc.deplete.pool.USE_MULTIPROCESSING = multiproc
    openmc.deplete.PredictorIntegrator(
        op, dt, power=174, timestep_units=time_units
    ).integrate()
