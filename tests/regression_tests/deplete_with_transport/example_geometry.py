...
def segment_pin(n_rings, n_wedges, r_fuel, r_gap, r_clad):
    ...
    fuel_wedges = [
        openmc.Plane(a=math.cos(theta[i]), b=math.sin(theta[i]))
        ...
        for i in range(n_rings):
            for j in range(n_wedges):
                ...
                if i == 0:
                    if j != n_wedges - 1:
                        cell.region = (
                            -fuel_rings[0] & +fuel_wedges[j] & -fuel_wedges[j + 1]
                        )
                    else:
...
def generate_geometry(n_rings, n_wedges):
    ...
    # Form pin cell
    fuel_u, v_segment, v_gap, v_clad = segment_pin(
        n_rings, n_wedges, r_fuel, r_gap, r_clad
    )
