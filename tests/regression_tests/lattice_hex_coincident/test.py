...
class HexLatticeCoincidentTestHarness(PyAPITestHarness):
    def __init__(self, *args, **kwargs):
        ...
        cyl = openmc.ZCylinder(x0=0.0, y0=0.0, r=pin_rad)
        ...
        edge_length = (2.0 / sqrt(3.0)) * half_width

        inf_mat = openmc.Cell()
        inf_mat.fill = matrix

        inf_mat_univ = openmc.Universe(
            cells=[
                inf_mat,
            ]
        )
        ...
        pincell_only_lattice.center = (0.0, 0.0)
...
def test_lattice_hex_coincident_surf():
    harness = HexLatticeCoincidentTestHarness("statepoint.5.h5", model=openmc.Model())
    harness.main()
