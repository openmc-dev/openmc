...
def model():
    ...
    x_max = openmc.XPlane(5.0, boundary_type="reflective")

    z_cyl = openmc.ZCylinder(x0=3 * cos(pi / 6), y0=3 * sin(pi / 6), r=2.0)

    outside_cyl = openmc.Cell(
        1, fill=water, region=(+plane1 & +plane2 & -x_max & +z_cyl)
    )
    inside_cyl = openmc.Cell(2, fill=fuel, region=(+plane1 & +plane2 & -z_cyl))
