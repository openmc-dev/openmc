...
def test_random_ray_auto_convert(method):
    with change_directory(method):
        ...
        model.convert_to_multigroup(
            ...
            temperature_settings=temp_settings, temperatures=[294.0, 394.0]
        )

        # Convert to a random ray model
        model.convert_to_random_ray()
        model.settings.temperature = temp_settings

        # Set all material temperatures to room temperature
        for mat in model.geometry.get_all_materials().values():
            mat.temperature = 294.0

        # Set the cell temperature of the fuel such that it moves up to the next
        # temperature bin.
        for cell in model.geometry.get_all_cells().values():
            if cell.name == "Fuel":
                cell.temperature = [395.0]
