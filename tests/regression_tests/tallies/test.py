...
def test_tallies():
    ...
    cellborn_tally.filters = [
        CellBornFilter(
            (
                model.geometry.get_all_cells()[10],
                model.geometry.get_all_cells()[21],
                22,
                23,
            )
        )
    ]  # Test both Cell objects and ids
    ...
    material_tally.filters = [
        MaterialFilter(
            (
                model.geometry.get_materials_by_name("UOX fuel")[0],
                model.geometry.get_materials_by_name("Zircaloy")[0],
                3,
                4,
            )
        )
    ]  # Test both Material objects and ids
    ...
    universe_tally.filters = [
        UniverseFilter(
            (
                model.geometry.get_all_universes()[1],
    ...
    ]  # Test both Universe objects and ids
    ...
    for t in score_tallies[0:2]:
        t.estimator = "tracklength"
    for t in score_tallies[2:4]:
        t.estimator = "analog"
    for t in score_tallies[4:6]:
