...
class TallySliceMergeTestHarness(PyAPITestHarness):
    def __init__(self, *args, **kwargs):
        ...
        low_energy = openmc.EnergyFilter([0.0, 0.625])
        high_energy = openmc.EnergyFilter([0.625, 20.0e6])
    ...
    def _get_results(self, hash_output=False):
        ...
        tallies = map(
            lambda tf: tf[0].get_slice(
                filters=[type(tf[1])], filter_bins=[(tf[1].bins[0],)]
            ),
            cell_filter_prod,
        )
        ...
        sum1 = distribcell_tally.summation(
            filter_type=openmc.DistribcellFilter, filter_bins=[0, 100, 2000, 30000]
        )
        # Sum up a few subdomains from the distribcell tally
        sum2 = distribcell_tally.summation(
            filter_type=openmc.DistribcellFilter, filter_bins=[500, 5000, 50000]
        )
        ...
        sum1 = mesh_tally.summation(
            filter_type=openmc.MeshFilter, filter_bins=[(1, 1), (1, 2)]
        )
        # Sum up a few subdomains from the mesh tally
        sum2 = mesh_tally.summation(
            filter_type=openmc.MeshFilter, filter_bins=[(2, 1), (2, 2)]
        )
