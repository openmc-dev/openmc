...
class TRISOTestHarness(PyAPITestHarness):
    def __init__(self, *args, **kwargs):
        ...
        spheres = [openmc.Sphere(r=r * 1e-4) for r in [212.5, 312.5, 347.5, 382.5]]
        ...
        trisos = [openmc.model.TRISO(outer_radius, inner_univ, c) for c in centers]

        # Create lattice
        ll, ur = box.region.bounding_box
        shape = (3, 3, 3)
        pitch = (ur - ll) / shape
        lattice = openmc.model.create_triso_lattice(trisos, ll, pitch, shape, graphite)
        box.fill = lattice
        ...
        self._model.materials = openmc.Materials(
