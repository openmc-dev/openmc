...
# Require numpy and pandas to print output rounded to 10 decimal places and in
...
np.set_printoptions(formatter={'float': lambda x: format(np.around(x, 10), 'f')})
pd.set_option('display.float_format', lambda x: '%f' % np.around(x, 10))
