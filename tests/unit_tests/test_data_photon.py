...
@pytest.mark.parametrize(
    ...
    [("H", "K", 13.61, 1.0), ("O", "L3", 14.15, 2.67), ("U", "P2", 34.09, 2.0)],
    indirect=["element"],
)
...
@pytest.mark.parametrize(
    ...
    [("H", 19.2, 0, 13.6, 1), ("O", 95.0, 2, 13.62, 4), ("U", 890.0, 25, 6.033, -3)],
    indirect=["element"],
)
...
@pytest.mark.parametrize(
    "element, n_shell", [("H", 1), ("O", 3), ("Al", 5)], indirect=["element"]
)
...
def test_export_to_hdf5(tmpdir, element):
    ...
    assert (
        element.compton_profiles["binding_energy"]
        == element2.compton_profiles["binding_energy"]
    ).all()
    assert (
        element.bremsstrahlung["electron_energy"]
