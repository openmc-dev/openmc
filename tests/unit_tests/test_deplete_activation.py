...
def model():
    ...
    model.settings.source = openmc.IndependentSource(
        space=openmc.stats.Point(), energy=openmc.stats.Discrete([1.0e6], [1.0])
    )
...
def test_activation(
    ...
    integrator = openmc.deplete.PredictorIntegrator(op, [t], source_rates=source_rates)
...
def test_flux_rr_missing_nuclide(run_in_tmpdir, model):
    ...
    # Deplete with two decay steps
    integrator = openmc.deplete.PredictorIntegrator(op, [100.0], source_rates=[10.0])
    integrator.integrate()
