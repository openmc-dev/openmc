...
from openmc.deplete.abc import (
    _SECONDS_PER_MINUTE,
    _SECONDS_PER_HOUR,
    _SECONDS_PER_DAY,
    _SECONDS_PER_JULIAN_YEAR,
)
...
@pytest.mark.parametrize(
    "case_name, transfer_rates",
    [
        ("elements", {"U": 0.01, "Xe": 0.1}),
        ("nuclides", {"I135": 0.01, "Gd156": 0.1, "Gd157": 0.01}),
        (
            "nuclides_elements",
            {"I135": 0.01, "Gd156": 0.1, "Gd157": 0.01, "U": 0.01, "Xe": 0.1},
        ),
        (
            "elements_nuclides",
            {"U": 0.01, "Xe": 0.1, "I135": 0.01, "Gd156": 0.1, "Gd157": 0.01},
        ),
        (
            "multiple_transfer",
            {"U": 0.01, "Xe": 0.1, "I135": 0.01, "Gd156": 0.1, "Gd157": 0.01},
...
def test_get_set(model, case_name, transfer_rates):
    ...
    material, dest_material, dest_material2 = [
        m for m in model.materials if m.depletable
    ]
    for material_input in [material, material.name, material.id]:
        for dest_material_input in [
            dest_material,
            dest_material.name,
            dest_material.id,
        ]:
            if case_name == "rates_invalid_1":
                ...
                with pytest.raises(
                    ValueError, match="Gb156 is not a valid " "nuclide or element."
                ):
                    for component, transfer_rate in transfer_rates.items():
                        transfer.set_transfer_rate(
                            material_input, [component], transfer_rate
                ...
                with pytest.raises(
                    ValueError, match="Gb is not a valid " "nuclide or element."
                ):
                    for component, transfer_rate in transfer_rates.items():
                        transfer.set_transfer_rate(
                            material_input, [component], transfer_rate
                ...
                for component, transfer_rate in transfer_rates.items():
                    ...
                    assert (
                        transfer.get_transfer_rate(material_input, component)[0]
                        == transfer_rate
                    )
                    assert transfer.get_destination_material(material_input, component)[
                        0
                    ] == str(dest_material.id)
                assert transfer.get_components(material_input) == transfer_rates.keys()

            if case_name == "multiple_transfer":
                for dest2_material_input in [
                    dest_material2,
                    dest_material2.name,
                    dest_material2.id,
                ]:
                    for component, transfer_rate in transfer_rates.items():
                        transfer.set_transfer_rate(
                            ...
                            [component],
                            transfer_rate,
                            destination_material=dest2_material_input,
                        )
                        for id, dest_mat in zip(
                            [0, 1], [dest_material, dest_material2]
                        ):
                            assert (
                                transfer.get_transfer_rate(material_input, component)[
                                    id
                                ]
                                == transfer_rate
                            )
                            assert transfer.get_destination_material(
                                material_input, component
                            )[id] == str(dest_mat.id)


@pytest.mark.parametrize(
    "transfer_rate_units, unit_conv",
    [
        ("1/s", 1),
        ("1/sec", 1),
...
def test_transfer(run_in_tmpdir, model):
    ...
    transfer_rate = 1e-5
    op = CoupledOperator(model, CHAIN_PATH)
    integrator = openmc.deplete.PredictorIntegrator(op, [1, 1], 0.0, timestep_units="d")
