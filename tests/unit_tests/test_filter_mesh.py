...
def test_spherical_mesh_estimators(run_in_tmpdir):
    ...
    sph_mesh = openmc.SphericalMesh(r_grid=np.linspace(0.0, 5.0**3, 20) ** (1 / 3))
    tally1 = openmc.Tally()
...
def test_cylindrical_mesh_estimators(run_in_tmpdir):
    ...
    cyl = openmc.model.RightCircularCylinder(
        (0.0, 0.0, -5.0), 10.0, 10.0, boundary_type="vacuum"
    )
    ...
    cyl_mesh = openmc.CylindricalMesh(
        r_grid=np.linspace(0.0, 5.0**3, 20) ** (1 / 3), z_grid=[-5.0, 5.0]
    )
    tally1 = openmc.Tally()
...
def test_cylindrical_mesh_coincident(scale, run_in_tmpdir):
    ...
    box = openmc.model.RectangularPrism(
        4 * scale, 4 * scale, boundary_type="reflective"
    )
    ...
    cyl_mesh = openmc.CylindricalMesh(
        r_grid=[0.0, 1.25 * scale], phi_grid=[0.0, 2 * math.pi], z_grid=[-1e10, 1e10]
    )
...
def test_spherical_mesh_coincident(scale, run_in_tmpdir):
    ...
    sph_mesh = openmc.SphericalMesh(
        r_grid=[0.0, 1.25 * scale],
        phi_grid=[0.0, 2 * math.pi],
        theta_grid=[0.0, math.pi],
    )
