...
def rlat2(pincell1, pincell2, uo2, water, zr):
    ...
    lattice.universes = [[u1, u2, u1], [u2, u1, u2], [u2, u1, u1]]
...
def rlat3(pincell1, pincell2, uo2, water, zr):
    ...
    lattice.outer = openmc.Universe(cells=[all_zr])
    lattice.universes = [
        [[u1, u2, u1], [u2, u1, u2], [u2, u1, u1]],
        [[u3, u1, u2], [u1, u3, u2], [u2, u1, u1]],
    ]

    # Add extra attributes for comparison purpose
    lattice.cells = [u1.fuel, u1.moderator, u2.fuel, u2.moderator, h_cell, all_zr]
...
def test_find(rlat2, rlat3, hlat2, hlat3):
    pitch = rlat2.pitch[0]
    seq = rlat2.find((0.0, 0.0, 0.0))
    assert seq[-1] == rlat2.cells[0]
    seq = rlat2.find((pitch, 0.0, 0.0))
    assert seq[-1] == rlat2.cells[2]
    seq = rlat2.find((0.0, -pitch, 0.0))
    assert seq[-1] == rlat2.cells[0]
    seq = rlat2.find((pitch * 100, 0.0, 0.0))
    assert seq[-1] == rlat2.cells[-1]
    seq = rlat3.find((-pitch, pitch, 5.0))
    assert seq[-1] == rlat3.cells[-2]

    pitch = hlat2.pitch[0]
    seq = hlat2.find((0.0, 0.0, 0.0))
    assert seq[-1] == hlat2.cells[2]
    seq = hlat2.find((0.5, 0.0, 0.0))
    assert seq[-1] == hlat2.cells[3]
    seq = hlat2.find((sqrt(3) * pitch, 0.0, 0.0))
    assert seq[-1] == hlat2.cells[0]
    seq = hlat2.find((0.0, pitch, 0.0))
    assert seq[-1] == hlat2.cells[2]

    # bottom of 3D lattice
    seq = hlat3.find((0.0, 0.0, -5.0))
    assert seq[-1] == hlat3.cells[2]
    seq = hlat3.find((0.0, pitch, -5.0))
    assert seq[-1] == hlat3.cells[2]
    seq = hlat3.find((0.0, -pitch, -5.0))
    assert seq[-1] == hlat3.cells[0]
    seq = hlat3.find((sqrt(3) * pitch, 0.0, -5.0))
    assert seq[-1] == hlat3.cells[0]
...
def test_indices_hex(hlat2, hlat3):
    ...
    assert hlat2.indices == (
        [(0, i) for i in range(12)] + [(1, i) for i in range(6)] + [(2, 0)]
    )
