...
def test_discretization_degenerate(rlat2):
    ...
    assert rlat_clone.get_universe((0, 0)).id != rlat_clone.get_universe((1, 0)).id
    assert rlat_clone.get_universe((1, 0)).id != rlat_clone.get_universe((0, 1)).id
    assert rlat_clone.get_universe((0, 1)).id != rlat_clone.get_universe((0, 0)).id
    assert rlat_clone.get_universe((0, 2)).id != rlat_clone.get_universe((2, 0)).id
    assert rlat_clone.get_universe((2, 1)).id != rlat_clone.get_universe((1, 2)).id


def test_discretization_skip_universe(rlat2):

    rlat_clone = rlat2.clone()
    rlat_clone.discretize(universes_to_ignore=[rlat_clone.get_universe((0, 0))])
...
def test_discretization_clone_only_some_materials(rlat2):
    ...
    assert (
        next(reversed(list(rlat_clone.get_universe((0, 0)).cells.values()))).fill
...
def test_discretization_lns_with_neighbor_list(rlat2):
    ...
    rlat_clone.discretize(
        strategy="lns", lattice_neighbors=[u1, u1, u2, u1, u2, u2, u1, u2]
    )
