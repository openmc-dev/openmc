...
def pin_model_attributes():
    ...
    fuel = openmc.Cell(cell_id=2, name="fuel", fill=fuel_inf_univ, region=-fuel_or)
    ...
    entropy_mesh.lower_left = [-0.39218, -0.39218, -1.0e50]
...
def test_init(run_in_tmpdir, pin_model_attributes, mpi_intracomm):
    ...
    assert sorted(test_model.settings.__dict__.keys()) == sorted(
        ref_settings.__dict__.keys()
    )
...
def test_from_xml(run_in_tmpdir, pin_model_attributes):
    ...
    assert (
        test_model.geometry.root_universe.cells.keys()
    ...
    assert sorted(
        k for k in test_model.settings.__dict__.keys() if k not in no_test
    ...
    assert test_model._materials_by_id == {
        1: test_model.materials[0],
        2: test_model.materials[1],
        3: test_model.materials[2],
    }
    assert test_model._materials_by_name == {
        "UO2": {test_model.materials[0]},
        "Zirc": {test_model.materials[1]},
...
def test_init_finalize_lib(run_in_tmpdir, pin_model_attributes, mpi_intracomm):
    ...
    c_mat = openmc.lib.find_material((0.6, 0.0, 0.0))
...
def test_import_properties(run_in_tmpdir, mpi_intracomm):
    ...
    # Now C
    assert openmc.lib.cells[1].get_temperature() == 600.0
...
def test_py_lib_attributes(run_in_tmpdir, pin_model_attributes, mpi_intracomm):
    ...
    # Now a good one
    assert np.all(openmc.lib.cells[2].rotation == (0.0, 0.0, 0.0))
    test_model.rotate_cells([2], (0, 0, 90))
    assert np.all(openmc.lib.cells[2].rotation == (0.0, 0.0, 90.0))
    ...
    assert np.all(openmc.lib.cells[2].translation == (0.0, 0.0, 0.0))
    test_model.translate_cells([2], (0, 0, 10))
    assert np.all(openmc.lib.cells[2].translation == (0.0, 0.0, 10.0))

    # Now lets do the density updates.
    # Check initial conditions
    assert openmc.lib.materials[1].get_density("atom/b-cm") == pytest.approx(
        0.06891296988603757, abs=1e-13
    )
    mat_a_dens = np.sum(
    ...
    # Change the density
    test_model.update_densities(["UO2"], 2.0)
    assert openmc.lib.materials[1].get_density("atom/b-cm") == pytest.approx(
        2.0, abs=1e-13
    )
    mat_a_dens = np.sum(
    ...
    assert mat_a_dens == pytest.approx(2.0, abs=1e-8)
    ...
    assert openmc.lib.cells[3].get_temperature() == pytest.approx(293.6, abs=1e-13)
    assert test_model.geometry.root_universe.cells[3].temperature is None
    # Change the temperature
    test_model.update_cell_temperatures([3], 600.0)
    assert openmc.lib.cells[3].get_temperature() == pytest.approx(600.0, abs=1e-13)
    assert test_model.geometry.root_universe.cells[3].temperature == pytest.approx(
        600.0, abs=1e-13
    )

    # And finally material volume
    assert openmc.lib.materials[1].volume == pytest.approx(
        0.4831931368640985, abs=1e-13
    )
    # The temperature on the material will be None because its just the default
    assert test_model.materials[0].volume == pytest.approx(
        0.4831931368640985, abs=1e-13
    )
    # Change the temperature
    test_model.update_material_volumes(["UO2"], 2.0)
    assert openmc.lib.materials[1].volume == pytest.approx(2.0, abs=1e-13)
    assert test_model.materials[0].volume == pytest.approx(2.0, abs=1e-13)

    test_model.finalize_lib()


def test_deplete(run_in_tmpdir, pin_model_attributes, mpi_intracomm):
    mats, geom, settings, tals, plots, op_kwargs, chain_file_xml = pin_model_attributes
    ...
    def check_tally_output():
        with openmc.StatePoint("openmc_simulation_n0.h5") as sp:
            ...
            fission = sp.get_tally(id=1).get_values(scores=["fission"])[0, 0, 0]
...
def test_calc_volumes(run_in_tmpdir, pin_model_attributes, mpi_intracomm):
    ...
    material_vol_calc = openmc.VolumeCalculation(
        [mats[2]],
        samples=1000,
        lower_left=(-0.63, -0.63, -100.0),
        upper_right=(0.63, 0.63, 100.0),
    )
    cell_vol_calc = openmc.VolumeCalculation(
        [geom.root_universe.cells[3]],
        samples=1000,
        lower_left=(-0.63, -0.63, -100.0),
        upper_right=(0.63, 0.63, 100.0),
    )
    test_model.settings.volume_calculations = [material_vol_calc, cell_vol_calc]
    ...
    assert mats[2].volume > 0.0
    assert geom.root_universe.cells[3].volume > 0.0
