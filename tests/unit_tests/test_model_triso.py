...
def centers_rectangular_prism():
    ...
    return openmc.model.pack_spheres(
        radius=_RADIUS, region=region, pf=_PACKING_FRACTION, initial_pf=0.2
    )
...
def test_triso_lattice(triso_universe, centers_rectangular_prism):
    trisos = [
        openmc.model.TRISO(_RADIUS, triso_universe, c)
...
def test_packing_fraction_input():
    ...
    with pytest.raises(ValueError):
        centers = openmc.model.pack_spheres(radius=_RADIUS, region=-openmc.Sphere(r=1))
