...
_PARAMS = [
    ...
    {'shape': 'x_cone', 'volume': 1/3*pi*(1**2 + 1*2 + 2**2)},
    {'shape': 'y_cone', 'volume': 1/3*pi*(1**2 + 1*2 + 2**2)},
    {'shape': 'z_cone', 'volume': 1/3*pi*(1**2 + 1*2 + 2**2)},
...
@pytest.fixture(scope='module')
def centers_x_cone():
    cone = openmc.XCone(x0=-1, y0=1, z0=2, r2=1)
    min_x = openmc.XPlane(0)
    max_x = openmc.XPlane(1)
...
@pytest.fixture(scope='module')
def centers_y_cone():
    cone = openmc.YCone(x0=1, y0=-1, z0=2, r2=1)
    min_y = openmc.YPlane(0)
    max_y = openmc.YPlane(1)
...
@pytest.fixture(scope='module')
def centers_z_cone():
    cone = openmc.ZCone(x0=1, y0=2, z0=-1, r2=1)
    min_z = openmc.ZPlane(0)
    max_z = openmc.ZPlane(1)
