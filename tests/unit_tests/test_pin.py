...
def test_subdivide_radially(pin_mats, good_radii, surf_type):
    ...
    # subdivide inner region radially
    N = 5
    div0 = pin_new(surfs, pin_mats, {0: N})
    assert len(div0.cells) == len(pin_mats) + N - 1
    ...
    # subdivide non-inner most region
    new_pin = pin_new(surfs, pin_mats, {1: N})
    assert len(new_pin.cells) == len(pin_mats) + N - 1
...
@pytest.mark.parametrize(
    "surf_type", [openmc.ZCylinder, openmc.XCylinder, openmc.YCylinder])
def test_subdivide_azimuthally(pin_mats, good_radii, surf_type):
    """Test the subdivision with various orientations"""
    surfs = [surf_type(r=r) for r in good_radii]
    fresh = pin_new(surfs, pin_mats, name="fresh pin")
    assert len(fresh.cells) == len(pin_mats)
    ...
    # subdivide inner regions azimuthally
    N = 5
    div0az = pin_new(surfs, pin_mats, None, {0:N})
    assert len(div0az.cells) == len(pin_mats) + N - 1
    
    # Check volume of fuel material
    for mid, mat in div0az.get_all_materials().items():
        if mat.name == "UO2":
            assert mat.volume == pytest.approx(100 / N)

    # subdivide non-inner most region
    div1az = pin_new(surfs, pin_mats, None, {1: N})
    assert len(div1az.cells) == len(pin_mats) + N - 1

    # Check volume of clad material
    for mid, mat in div1az.get_all_materials().items():
        if mat.name == "zirc":
            assert mat.volume == pytest.approx(100 / N)
...
@pytest.mark.parametrize(
    "surf_type", [openmc.ZCylinder, openmc.XCylinder, openmc.YCylinder])
def test_subdivide_both(pin_mats, good_radii, surf_type):
    """Test the subdivision with various orientations"""
    surfs = [surf_type(r=r) for r in good_radii]
    fresh = pin_new(surfs, pin_mats, name="fresh pin")
    assert len(fresh.cells) == len(pin_mats)
    ...
    # subdivide inner region both azimuthally and radially
    Nr = 5
    Na = 5
    div0b = pin_new(surfs, pin_mats, {0:Nr}, {0:Na})
    assert len(div0b.cells) == len(pin_mats) + Nr*Na - 1

    # Check volume of fuel material
    for mid, mat in div0b.get_all_materials().items():
        if mat.name == "UO2":
            assert mat.volume == pytest.approx(100 / (Nr*Na))
