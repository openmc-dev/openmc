...
def test_zernike():
    ...
    norm_vec = np.array([1, 4, 4, 6, 3, 6, 8, 8, 8, 8, 10, 10, 5, 10, 10]) / (
        np.pi * 0.392**2
    )
    norm_coeff = norm_vec * coeff
    ...
    ref_vals = [
        np.sum(norm_coeff * raw_zn1),
        np.sum(norm_coeff * raw_zn2),
        np.sum(norm_coeff * raw_zn3),
        np.sum(norm_coeff * raw_zn4),
    ]
