...
configured setting -- "auto" is resolved into a separate run-scoped value --
but the configured setting is a static that XML parsing only assigns when
the element is present, so openmc_finalize_random_ray() must restore the
"auto" default between runs. Running an explicit estimator first and a
default model second makes a missed restore visible: the second run would
report the first run's estimator instead of resolving "auto". The default
second run is an adjoint solve, which also pins the auto routing under
openmc.lib (it must resolve to the strict adaptive estimator, once per
solve)."""
...
def test_random_ray_default_estimator_persistence(run_in_tmpdir, capfd):
    ...
    # The forward run reports once; the adjoint run reports once per solve
    # (forward-for-adjoint, adjoint).
