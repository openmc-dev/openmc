"""Tests for GUI-facing slice plot overlap diagnostics.
...
def _run_overlap_slice(model):
    ...
    geom_data, _ = openmc.lib.slice_plot(
        origin=(0.0, 0.0, 0.0),
        width=(2.0, 2.0),
        pixels=(20, 20),
        basis="xy",
...
def _overlap_records(x, y):
    ...
    cell1, cell2, universe = openmc.lib.slice_plot_overlap_data(x, y)
    return list(zip(cell1.tolist(), cell2.tolist(), universe.tolist()))


def test_slice_plot_overlap_data_reports_cells_and_universe(
    ...
    with openmc.lib.TemporarySession(
        ...
        assert openmc.lib.slice_plot_overlap_count(x, y) == 1
...
def test_slice_plot_overlap_data_reports_triple_overlap_pairs(
    ...
    with openmc.lib.TemporarySession(
        ...
        assert openmc.lib.slice_plot_overlap_count(x, y) == 2
...
def test_slice_plot_overlap_data_is_empty_for_non_overlap_pixel(
    ...
    with openmc.lib.TemporarySession(
        ...
        assert openmc.lib.slice_plot_overlap_count(x, y) == 0
